...
/obj/item/implant/dead
	name = "Dead Man's switch"
	icon_state = "implant-r"
	impcolor = "r"
	var/activation_emote = "deathgasp"
	var/obj/item/device/radio/signaler/my_signaller
	New()
		..()
		my_signaller = new()
		src.my_signaller = my_signaller
		my_signaller.set_loc(src)
		my_signaller.master = src

	implanted(mob/source as mob)
		..()
		if(source.mind)
			source.mind.store_memory("Your Dead Man's Switch(tm) implant will activate upon death.", 0 , 0)
		boutput(source, "The implanted Dead Man's Switch(tm) implant will activate upon death.")

	trigger(emote, mob/source as mob)
		if (!source || (source != src.loc) || (source.stat != 2 && prob(99)))
			return
		if (emote == src.activation_emote)
			source.visible_message("[source] emits a faint radio crackle.")
			my_signaller.send_signal("ACTIVATE")
			logTheThing("signalers", !usr && src.master ? src.master.fingerprintslast : usr, null, "Triggered a dead man's switch at [src.master ? "[log_loc(src.master)]" : "[log_loc(src)]"]. Frequency: [format_frequency(my_signaller.frequency)]/[my_signaller.code].")
			return


/obj/item/implant/dead/attack_self(mob/user as mob, flag1)
	my_signaller.attack_self(usr)
...
// allows removal of implant stored by clicking on it with an empty hand
	attack_hand(mob/user as mob)
		if (src.loc == user && src.imp)
			user.put_in_hand_or_drop(src.imp)
			boutput(user, "You take [src.imp] out of [src].")
			src.imp = null
...
/obj/item/implantcase/dead
    name = "glass case - 'Dead Man's Switch'"
    implant_type = "/obj/item/implant/dead"
